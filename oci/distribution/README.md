...
| `auth.htpasswd.*` | Built-in basic auth ships with user `oci` / `JQdMvN43Qkfo9IoFOTHW94`. Replace the plaintext + bcrypt hash in `values.yaml`, add more `entries`, or point to `existingSecret` for production. |
| `registryCredentials.*` | When enabled, renders a `kubernetes.io/dockerconfigjson` secret (default `<release>-distribution-dockerconfig`) preloaded with the first htpasswd entry so you can `kubectl get secret ... -ojsonpath='{.data.\\.dockerconfigjson}' | base64 -d` to retrieve the username/password pair. Set `server` if you want a hostname other than the chart’s service DNS. |
